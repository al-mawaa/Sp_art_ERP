...
export default function BatchAttendancePage() {
    ...
    const fetchBatch = async () => {
      try {
        ...
        if (data.attendance?.students) {
          ...
          toast({ title: "Attendance loaded", description: "Existing records loaded for this date." });
        }

        setAttendance(initialAttendance);
      } catch {
        toast({ title: "Error", description: "Unable to load batch attendance.", variant: "destructive" });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRemarkChange = (studentId: string, remark: string) => {
    setAttendance(prev => ({ ...prev, [studentId]: { ...prev[studentId], remark } }));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!students.length) {
      toast({
        ...
        description: "Mark at least one student as Present or Absent.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Error saving attendance", description: "Please try again.", variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };

  if (!batchId) return <div className="p-4">Batch not specified.</div>;
  ...
  return (
    <div className="space-y-6 p-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <Button variant="ghost" onClick={() => router.push("/teacher/student-attendance")}>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <Button onClick={handleSubmit} disabled={submitting || selectedRows === 0} className="rounded-3xl px-6 py-3">
