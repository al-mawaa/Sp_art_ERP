...
/** GET batches list, or ?batchId=&date= for existing record */
export async function GET(request: NextRequest) {
  try {
    ...
    const batches = await listAssignedBatchesForStaff("teacher", auth.teacher.id);
    return NextResponse.json({ success: true, data: { batches } });
  } catch (e) {
    console.error("[teacher/self-attendance GET]", e);
    return NextResponse.json({ success: false, error: "Failed to load attendance" }, { status: 500 });
  }
}

/** POST mark own attendance */
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("[teacher/self-attendance POST]", e);
