...
export async function GET(request: NextRequest) {
  try {
    ...
    const requests = await InventoryRequest.find({
      requesterId: new mongoose.Types.ObjectId(auth.student.id),
    ...
    console.error("Error fetching inventory requests for student:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Error creating inventory request for student:", error);
